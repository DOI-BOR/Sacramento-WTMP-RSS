...
def initRuleScript(currentRule, network):
	...
	# WQ is being simulated
		...
		engineAdapter.setShastaTCDInfo(sdId, tcdId, lowerGateIdx, middleGateIdx, upperGateIdx)
	...
	if evalRule:
		...
		if penstockFlow < 1.0:  # 1 cfs
			...
			if not isValidValue(prevLevel):
				# default to highest level if this happens on first time step
				prevLevel = getElevationRestrictionLevel(currentRule, network, resElev)
...
def getForecastTCDTempAndFlows(currentRule, network, currentRuntimestep, resElev, targetTemp, tcdMinFlow, 
	...
	nGateOptions = len(getNumGateOptions())
	...
	if iCurStep <= 1:
		...
		if prevGateLevel < 1 or prevGateLevel > nGateOptions:
			prevGateLevel = elevRestrictionLevel
		...
		if curHour != checkOpHour:
			...
			# Outside of operation period - just don't switch gates too often
